 ...
 *
 *
 * GCDAsyncSocket使用标准的代理范例，但是会在给定的代理dispatch_queue队列中执行所有的代理回调。
 * 这允许最大限度的并发，同时保证比较简便的线程安全。
 *
 * 在使用socket前你必须设置delegate和delegateQueue，否则会报错。
 *
 * socketQueue是可选的。
 * 如果你传入NULL，GCDAsyncSocket会自动创建。
 * 如果你选择提供一个socketQueue，那么这个socketQueue不能是一个并发队列。
 * 如果你选择提供一个socketQueue，并且这个socketQueue有一个配置好target的队列，请查看关于markSocketQueueTargetQueue方法的说明。
 *
 * delegateQueue和socketQueue可以相同。
**/
- (instancetype)init;
...
/*
 * 如果你需要在代理的dealloc方法中将delegate置为nil，请使用下面提供的同步方法。
 */
...
/*
 * 默认支持IPv4和IPv6。
 * 对于接收连接，GCDAsyncSocket自动支持两种协议，并且支持只含有单协议的连接。
 * 对于外发连接，GCDAsyncSocket可以连接使用任一协议的远端主机。
 * 如果DNS域名解析只返回了IPv4结果，自动使用IPv4。
 * 如果DNS域名解析只返回了IPv6结果，自动使用IPv6。
 * 如果DNS域名解析同时返回了IPv4和IPv6结果，将会使用优先协议。
 * 优先协议默认为IPv4，但是可以重新设置。
 */
...
/*
 * 当使用Happy Eyeballs (RFC 6555)去连接IPv4和IPv6时，下面alternateAddressDelay这个参数表示使用优先协议和备用协议的延迟。
 * 默认值为300ms
 * 理解为：延迟超过300ms切换为备用协议方式。
 */
...
/*
 * userData允许你使用socket关联任何信息，在任何情况下这个数据都不会被socket在内部使用。
 */
...
/*
 * 告知socket开始监听并接收给定端口的连接。
 * 当一个连接被接收，一个新的GSCAsyncSocket实例将会产生来操作它，并且socket:didAcceptNewSocket:代理方法会被调用。
 *
 * socket会在所有可用连接方式上进行监听（例如wifi、以太网等）。
 */
...
/*
 * 该方法和acceptOnPort:error一样，只是额外增加了一个指定监听哪种连接方式的选择。
 *
 * 例如，你可以指定socket只接收以太网的连接，但不接收其他方式的连接如wifi。
 *
 * 这个连接可以通过名称（例如“en1”或者‘lo0’）或IP地址（例如192.168.4.34）指定。
 * 你也可以使用特定的字符串“localhost”或者“loopback”来制定socket只接受来自本机的连接。
 *
 * 你可以使用命令行工具“ifconfig”查看连接列表，或者使用getIfAddrs()方法查看。
 *
 * 想要接收任何连接的信息，该值传nil，或者用上述acceptOnPort:error:方法。
 */
